...
class productListPDF extends FPDF
{
   ...
   public function generateproductList()
   {
        ...
        foreach ($products as $product) {
            ...
            if (isset($product['Id']) && isset($product['Title']) && isset($product['Barcode']) && isset($product['Category']) && isset($product['Brand']) && isset($product['Price']) && isset($product['Rating']) && isset($product['Stock'])) {
                $this->Cell(5); // Left margin
                $this->Cell(10, 10, $product['Id'], 1); // Assuming 'Id' is the unique identifier
